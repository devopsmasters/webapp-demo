node {
   ...
   stage('Build') {   
	...
	task 'Run Integration'
		  ...
		  sh 'mvn -Dtest=SeleniumIntegration clean test -Dwebdriver.type=remote -Dwebdriver.url=http://zalenium:4444/wd/hub -Dwebdriver.cap.browserName=chrome -Dmaven.test.failure.ignore=true'
	        }
              }
            }
	    finally {
